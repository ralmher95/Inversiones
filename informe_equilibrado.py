"""
Informe Cartera Conservadora 5 ETFs - Word (sin scipy)
Ejecutar: python informe_word_5activos_light.py
"""
import pandas as pd
import numpy as np
...
def stats_cartera(w):
    ...
    ret = port_ret.mean() * N_DIAS
    vol = port_ret.std() * np.sqrt(N_DIAS)
    if vol == 0:
...
run = sub.add_run('Cartera Conservadora Global (5 ETFs)')
run.font.size = Pt(12)
...
doc.add_paragraph('Referencia basada en simulaciones históricas. Los pesos reales se muestran en la sección 5.')
add_table(
    ['Activo', 'Rol'],
    [
        ['EUNA.DE (Global Bonds)', 'Estabilizador principal'],
        ['XMWO.DE (MSCI World Swap)', 'Crecimiento renta variable'],
        ['IQQ0.DE (Min Volatility)', 'Renta variable defensiva'],
        ['SGLD.L (Physical Gold)', 'Cobertura inflación / crisis'],
        ['IS3N.DE (Emerging Markets)', 'Exposición emergente'],
    ]
)
...
add_table(
    ['Activo', 'Peso (%)'],
    [
        ['Global Bonds (EUNA.DE)', '50'],
        ['Min Volatility (IQQ0.DE)', '20'],
        ['Physical Gold (SGLD.L)', '15'],
        ['Emerging Markets (IS3N.DE)', '10'],
        ['MSCI World Swap (XMWO.DE)', '5'],
    ]
)
...
doc.add_paragraph(
    'La cartera de Máximo Sharpe es la más equilibrada. '
    'La de Mínima Volatilidad es ideal para conservadores. '
    'Se recomienda rebalanceo semestral.'
)
...
doc.save('Informe_Equilibrado.docx')
print("✅ Documento Word generado: Informe_Equilibrado.docx")